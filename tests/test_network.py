...
class TestNotifyVpnUnprotected:
    def test_sends_notification_tunnel_down(self):
        with mock.patch("subprocess.run") as mock_run:
            netbuoy.notify_vpn_unprotected(tunnel_up=False)
            mock_run.assert_called_once()
            cmd = mock_run.call_args[0][0]
            assert cmd[0] == "osascript"
            assert "VPN is down" in cmd[2]
            assert "Basso" in cmd[2]

    def test_sends_notification_tunnel_up_but_unprotected(self):
        with mock.patch("subprocess.run") as mock_run:
            netbuoy.notify_vpn_unprotected(tunnel_up=True)
            mock_run.assert_called_once()
            cmd = mock_run.call_args[0][0]
            assert "tunnel is up but your IP is NOT protected" in cmd[2]

    def test_handles_osascript_not_found(self):
        with mock.patch("subprocess.run", side_effect=FileNotFoundError()):
            netbuoy.notify_vpn_unprotected()  # Should not raise
